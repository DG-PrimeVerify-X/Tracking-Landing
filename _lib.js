...
function cryptoHash(s) {
  // Web Crypto is available in the Vercel Node runtime.
  // A compact deterministic non-reversible identifier is sufficient here.
  let h = 2166136261;
...
export function classify(req, body, recentClicks=[]) {
  const ua = req.headers["user-agent"] || "";
  const reasons = [];
  if (!ua || /bot|crawler|spider|headless|selenium|playwright|phantom/i.test(ua))
    reasons.push("bot-like user agent");

  const dwell = Number(body.dwellSeconds || 0);
  if (dwell > 0 && dwell < 1) reasons.push("sub-second dwell");

  if (recentClicks.length >= 6) reasons.push("high click frequency");

  if (reasons.length) return { label:"suspicious", reasons };
  return { label:"likely_real", reasons:[] };
}
